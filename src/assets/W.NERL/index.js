...
export default {
  ...
  description: [
    ...
    'Never mind the fact that I didn’t know Python nor databases. What challenges could a dreamer, believer, and heroic knight possible run into? I will tell you. The misunderstood giant, by the name of ‘Wikipedia,’ stood, at first, as the obstacle to overcome and then became the friend that took lots of efforts to handle. But no effort is ever wasted and this friendship produced the most amazing gems. Literal gems.',
    'Okay, enough of that. I apologize. For more imaginative adventures, contact me and we may let you join our DnD campaign.',
    'I wanted to create a suggestion engine that would take one Wikipedia article as the input and it would produce a number of well related and important articles that the user could read to continue their study. We already had the data - that was all of Wikipedia. I concluded that I could apply a DIY version of Google’s PageRank algorithm to every page and link that existed in Wikipedia to find achieve my goals. Spoiler alert: it worked really well.',
    {
      type: 'quote',
    ...
    'Let’s say there are 40 people on an island and we want to know who is the most influential. Some popular candidates would be Dan, Blaine, Lacey, and George, all of which have the highest number of friends (18, 13, 12, and 9 respectively). For some reason though, all 4 of these people really, really like Brady. Brady doesn’t have that many friends but he is really close with these four and they do hangout exclusively a lot. Now, when Brady has this new idea on how to climb trees, his friend group follows suit and they all start doing it. Only then do the friends of Dan, Blaine, etc. follow suit because the ‘popular’ kids are doing it. Thus we see that Brady is the most influential and the trendsetter. It’s not the number of his friends that’s important. It’s the fact that his friends have a lot of friends. Or as too many people have originally said (tldr;) quality > quantity.',
    'Here’s a more technical version of the same thing: in a graph, we have lots of nodes and edges (or pages and links, but this is the only time I’m going to say those words). We first count how many incoming edge each node has and that becomes it’s temporary weight. We then go through the entire graph again but this time we multiply the incoming edge with the weight of the node it is coming from and then sum up all the incoming edges. We do that with each node and that becomes the node’s new weight/importance/centrality.',
    'The idea is simple. The more think point to you, the more important your opinion must be. So when you point to something, that point is much more weighty then the previous points to you.',
    'Did I explain that too many times? Good. Feel smart, that’s how Google worked at first and to some degree still does. ',
    'The final part to this was simple. When I wanted suggestions for a given node, I would use all the outgoing edges to nodes and sort those nodes by their centrality. Return the top 5. I’m not kidding, this worked really well.',
    {
      type: 'bigHeader',
    ...
    'Hahaha, just kidding. Turns out, ‘United States’ has the most incoming edges by a ludicrous amount; it had 383,510 incoming nodes. The next highest was barely half that at 194,868 (Animal). In fact, the top 438 (of 5,301,396) nodes held 10% of all incoming edges. Consequently, on virtually any search I did, some random nodes that was pointed to by the ‘United States’ node would end up being the winners and this was not helpful.',
    'So what did I do? I broke statistical law. Believe me, I tried z-score normalization and the left ‘United States’ with a score of almost 1600! That doesn’t fix anything! I tried some other methods to normalize to no avail. So finally, I just 8th rooted all the weight. I know, it’s bad. In my defense though, it kept the minimum of 1 at 1 but brought the max down to around 5. Now, an ‘important’ edge could be outclassed by lots of less important edges. Democracy!',
    ...
    'So, starting back at the initial weight and my subpar normalizing technique, I reapplied the algorithm and this time, I was happy with the results. I didn’t keep getting ‘Germany’ pop up as the suggestion to everything, so that alone was improvement. Not that there’s anything wrong with ‘Germany’ but obviously ‘Germany’ isn’t the answer to absolutely everything.',
    {
      type: 'smallHeader',
